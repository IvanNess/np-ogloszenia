...
import { administracja, budownictwo, callcenter, finanse, edukacja, grafika, hotelarstwo, itadministracja, itprogramowanie, 
    inżynieria, kadry, lancuchDostaw, marketing, media, medycyna, nieruchomosci, fizyczna, offshore, produkcja, prawo, 
    sztuka, sprzedaz, transport, zdrowie, agencjePracy, analizaStatystyka, kontrolaYakosci, naprawa, ubezpieczenia, choicesArr
} from '../../constants';
...
const SecondWindow = ({max}) => {
    ...
    useEffect(()=>{
        if(errors.includes('categoriesValues')){
            setIsError(true)
        } else{
            setIsError(false)
        }
    ...
    return (
        <>
            <div className={`secondWindow ${isError && 'error'} categoriesValues`} onClick={open}>
                <MultSelectHeader choice="categoriesValues" isOpen={false} isInner={false} isSingleValue={false} title="Wyberz" choiceValues={choicesArr}/>
                {isOpen && <div>
                        ...
                        <div className='window' ref={ref}>
                            ...
                            <div className='body'>
                                <div className='col'>
                                    ...
                                    <ChoiceValue choice='agencjePracy' value={agencjePracy} key='Agencje pracy' isInner={false} isSingleValue={false} search={search} max={max}/>
                                    <ChoiceValue choice='analizaStatystyka' value={analizaStatystyka} key='Analiza Statystyka' isInner={false} isSingleValue={false} search={search} max={max}/>
                                ...
                                <div className='col'>
                                    ...
                                    <ChoiceValue choice='kontrolaYakosci' value={kontrolaYakosci} key='Kontrola jakości / Audyt' isInner={false} isSingleValue={false} search={search} max={max}/>
                                    ...
                                    <ChoiceValue choice='naprawa' value={naprawa} key='Naprawa / Serwis / Usługi' isInner={false} isSingleValue={false} search={search} max={max}/>
            ...
            <MyError choice='categoriesValues'/>
        </>
    )
}
